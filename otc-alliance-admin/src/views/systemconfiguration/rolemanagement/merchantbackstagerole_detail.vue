...
import {
  ...
  BusinessRoleGetListAll,
  BusinessRoleSetRoleMenus,
...
const fetchUserPermissionList = async () => {
  BusinessRoleGetListAll.params = {
    roleID: route.params.id,
  };
  const { result } = await request(BusinessRoleGetListAll);
...
const handleSave = async () => {
  BusinessRoleSetRoleMenus.data = {
    roleID: route.params.id,
    menuIds: checkedKeys.value,
  };
  const { result } = await request(BusinessRoleSetRoleMenus);
  if (result) {
