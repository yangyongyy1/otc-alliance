<template>
  <div class="order-detail-container" v-loading="loading">
    ...
    <div class="detail-content" v-if="detailData">
      <!-- 订单信息（按图片：平台订单、渠道订单、所属联盟、关联对象、用户类型、用户名称、用户邮箱、交易币种、交易方式、交易金额、交易状态、创建时间、更新时间） -->
      <el-descriptions :title="t('order.orderInfo')" :column="2" border>
        ...
        <el-descriptions-item :label="t('order.detailUserType')">{{ getUserTypeLabel(detailData.order?.userType) }}</el-descriptions-item>
      ...
      <!-- 收付款人信息先隐藏 -->
      <template v-if="false && detailData.detail">
...
const initOptions = async () => {
  try {
    const statusRes = await getEnum("CollectionOrderStatus");
...
const getUserTypeLabel = (userType) => {
  ...
  if (userType === 0) return t("order.userTypeIndividual");
  if (userType === 1) return t("order.userTypeEnterprise");
  return "--";
};
